/*
 *  This is standard java BufferedReader with a minor change to
 *  keep count of CR and LF chars scanned by ReadLine method.
 */
...
public class BufferedReaderCrLf {
    ...
    private int lastBytesConsumed = 0;
...
//	super(in);
     ...
     * @param in A Reader
     */
    public BufferedReaderCrLf(InputStream in) {
...
//                        System.arraycopy(cb, startChar, s, s.length, i - startChar);
//			s.append(Arrays.copyOfRange(cb, startChar, i - startChar).toString());
                        str = s.toString();
                    }
                    lastBytesConsumed += i - startChar;
                    lastBytesConsumed++;
                    nextChar++;
                    charsSkippedOnReadLine++;
                    if (c == '\r') {
                    ...
                    charsSkippedOnReadLine += skipped;
                    if (skipped > 0) {
...
//                s.append(Arrays.copyOfRange(cb, startChar, i - startChar).toString());
    ...
    String readBytes(int cnt) throws IOException {
        BArray s = null;
        int startChar;
        charsSkippedOnReadLine = 0;
        lastBytesConsumed = 0;
        ...
        Main.logger.trace("Need to read " + cnt + " bytes.");
        synchronized (lock) {
            ensureOpen();
...
//		    }
                ...
                if (cnt == 0) { // read necessary number of bytes
                    String str;
                    if (s == null) {
                        str = new String(cb, startChar, i - startChar);
                    } else {
...
//                s.append(Arrays.copyOfRange(cb, startChar, i - startChar).toString());
    ...
    class BArray {
        ...
        public void append(byte[] buf, int start, int num) {
            if ((sPos + num) > arr.length) {
                byte[] s1 = new byte[sPos];
                System.arraycopy(arr, 0, s1, 0, sPos);
                arr = new byte[sPos + num];
                System.arraycopy(s1, 0, arr, 0, sPos);
            }
//		s.append(cb, startChar, i - startChar);
            System.arraycopy(buf, start, arr, sPos, num);
            sPos += num;
        }

        @Override
        public String toString() {
            return new String(arr, 0, sPos);
        }
    }
}
