...
//  -----
    ...
    public VOIPEPParser(HashMap<TableType, DBTable> m_tables) {
        ...
        VOIPEPParser.ifSIPLinesForce = Main.ifSIPLines();
                        ...
                        if (nR == 0) {//workaround for FTP log parsing that strips 0x0d
                            if (!gaveWarning) {
                                String msg = "No \\n(0x0d) character in SIP header; Log file probably corrupt when file transferred\n"
                                        ...
                                        + "To force SIP by-line reading, use -DSIPLINES=1 in the command line";
//                                JOptionPane.showMessageDialog(null, msg, "File corrupt", JOptionPane.INFORMATION_MESSAGE);
