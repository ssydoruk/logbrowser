...
class AllCalls_GRESettings extends AllInteractionsSettings {

    private final GREServerResults ixnResults;
    private final SmartButtonGroup<IGetAllProc> selectAllReportTypeGroup;
    private static AllCalls_GRESettings INSTANCE = null;

    public static AllCalls_GRESettings getInstance(GREServerResults _ixnResults, Window c) {
        if (INSTANCE == null) {
            INSTANCE = new AllCalls_GRESettings(_ixnResults, c);
        }
        return INSTANCE;
    }
    ...
    public AllCalls_GRESettings(GREServerResults _ixnResults, Window parent) {
        super(parent);
