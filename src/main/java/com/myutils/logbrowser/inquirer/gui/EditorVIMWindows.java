...
public class EditorVIMWindows extends ExternalEditor {
...
//            logger.info("Waiting for process to stop");
    ...
    private static String getVIM_Text(String sVIMPath) {
        return FilenameUtils.getFullPath(sVIMPath) + "vim.exe";
    }

    private static String getVIM_GUI(String sVIMPath) {
        return FilenameUtils.getFullPath(sVIMPath) + "gvim.exe";
    }
    static Process proc = null;

    private static void ensureRunning() throws IOException {
        ArrayList<String> params = new ArrayList<>(2);
        params.addAll(Arrays.asList(getVIM_GUI(inquirer.getCr().getLinuxEditor()),
                "--servername",
                gvimServer));

        if (proc == null) {
            proc = Runtime.getRuntime().exec(params.toArray(new String[params.size()]), null, null);
        }

    }

    private static List<String> makeExecCmd(List<String> cmd) throws IOException {
        ensureRunning();
        ArrayList<String> params = new ArrayList<>(3 + ((cmd == null) ? 0 : cmd.size()));
        params.addAll(Arrays.asList(getVIM_Text(inquirer.getCr().getLinuxEditor()),
                "--servername",
                gvimServer));
        if (cmd != null) {
            params.addAll(cmd);
...
//        }
            ...
            Logger.getLogger(EditorVIMWindows.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
