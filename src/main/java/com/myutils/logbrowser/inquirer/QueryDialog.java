...
public class QueryDialog extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jpTabsLayout.setHorizontalGroup(jpTabsLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                .addGap(0, 910, Short.MAX_VALUE));
        jpTabsLayout.setVerticalGroup(jpTabsLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
...
//            fixOversizedWindow(_querySetting);
    ...
    private void btOKActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btOKActionPerformed
        try {
            ...
            inquirer.logger.info("Executing request [" + theQuery.getName() + "] search [" + theQuery.getSearchString()
                    + "] sum[" + theQuery.getReportSummary() + "]");
                ...
                if (getall != null) {
                    class QueryAllTask extends MySwingWorker<Void, String> {
                        ...
                        protected void done() {
                            ...
                            rp.dispose();
                            if (isCancelled()) {
                                JOptionPane.showMessageDialog(frm, "Query was cancelled", "Error",
    ...
    private void fileOutputConfig(QueryTools qry) {
        ...
        if (isAggregate) {
            ...
            fileOutSetting
                    .setOutDir(inquirer.getConfig().getFileNameLong(((IQueryResults) qry).getName(), getSelection()));
    ...
    private JideTabbedPane createTabbedPane() throws Exception {
        ...
        for (int i = 0; i < tabbedPane.getTabCount(); i++) {
            ...
            // inquirer.logger.info("rp.getQry().getApps().isEmpty()
            // "+rp.getQry().getApps().isEmpty());
        ...
        tabbedPane.setTabEditingValidator(new TabEditingValidator() {
            ...
            public boolean alertIfInvalid(int tabIndex, String tabText) {
                if (tabText.trim().length() == 0) {
                    JideOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(tabbedPane),
                            "'" + tabText + "' is an invalid name for a tab title.", "Invalid Tab Title",
                            JideOptionPane.ERROR_MESSAGE, null);
                    return false;
                }

                for (int i = 0; i < tabbedPane.getTabCount(); i++) {
                    if (tabText.trim().equalsIgnoreCase(tabbedPane.getDisplayTitleAt(i))
                            && i != tabbedPane.getSelectedIndex()) {
                        JideOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(tabbedPane),
                                "There is already a tab with the title of '" + tabText + "'.", "Invalid Tab Title",
                                JideOptionPane.ERROR_MESSAGE, null);
                        return false;
                    }
                }
    ...
    private void tabChanged(ChangeEvent e) {
        ...
        RequestParams currentRequest = getCurrentRequest();
        boolean selEnabled = (currentRequest != null
                && currentRequest.getSelectionType() != SelectionType.NO_SELECTION);
                ...
                if (ps.getNrows() > 0 || ps.getTabDataModel().getRowCount() > 0) {
                    ...
                    if (objectsFound == 0) {
                        JideOptionPane.showMessageDialog(queryDialog,
                                "Nothing found.\n" + "Try changing search parameters", "cannot run report",
                                ERROR_MESSAGE);
                    } else {
                        JideOptionPane.showMessageDialog(queryDialog,
