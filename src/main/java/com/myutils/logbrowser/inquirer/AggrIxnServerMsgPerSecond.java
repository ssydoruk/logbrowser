 ...
 * @author ssydoruk
 */
public class AggrIxnServerMsgPerSecond extends IAggregateAggregate {

    private boolean isTLibReport;

    public AggrIxnServerMsgPerSecond() {
        setJpConfig(new AggrIxnServerMsgPerSecondConfig());
    }
...
//        select datetime(time/1000,'unixepoch', 'localtime') ts,* from ocs_logbr
        AggrIxnServerMsgPerSecondConfig cfg = (AggrIxnServerMsgPerSecondConfig) getJpConfig();
//        setMaxRecords(2000);
...
//        Sort();
    ...
    private void runReport(AggrIxnServerMsgPerSecondConfig cfg, Integer[] appIDs) throws SQLException {
        isTLibReport = true;
        DynamicTreeNode<OptionNode> attrRoot = cfg.getAttrTLibRoot();
        Wheres wh = new Wheres();
        DynamicTreeNode<OptionNode> node = FindNode(qd.getSelectedQuery().getReportItems().getRoot(), DialogItem.IXN, DialogItem.IXN_EVENT, null);
        wh.addWhere(IQuery.getCheckedWhere("nameID",
                ReferenceType.TEvent,
                node,
                DialogItem.IXN_IXN_EVENT_NAME, true), "AND");
        wh.addWhere(IQuery.getCheckedWhere("ixnqueueid",
                ReferenceType.DN,
                node,
                DialogItem.IXN_IXN_EVENT_DN, true), "AND");
        wh.addWhere(IQuery.getCheckedWhere("strategyID",
                ReferenceType.URSStrategyName,
                node,
                DialogItem.URS_STRATEGY, true), "AND");
...
//        wh.addWhere(IQuery.getAttrsWhere("strategyid", ReferenceType.URSStrategyName, FindNode(attrRoot, DialogItem.URS_STRATEGY, null, null)), "AND");
        ...
        wh.addWhere(TableType.Ixn, "time", qd.getTimeRange(), "AND");
        ...
        for (DBField fld : grpBy) {
            DatabaseConnector.safeCreateIndex(tab, fld.getFieldName());
        }
