...
public class CallFlowResults extends IQueryResults {
    ...
    // private static final String[] RequestsToShow = {"RequestMakePredictiveCall",
    // "RequestMakeCall", "RequestMonitorNextCall"};
    // private static final String[] EventsToShow = {"EventDialing",
    // "EventNetworkReached", "EventMonitoringNextCall"};
    ...
    FullTableColors getAll(QueryDialog qd, AllInteractionsSettings settings) throws SQLException {
        try {
            ...
            DatabaseConnector.runQuery("create temp table " + tmpTable + " (" + "connectionidid int"
                    + ",started timestamp" + ",ended timestamp" + ",thisdnid int" + ",otherdnid int" + ",aniid int"
                    + ",nameid int" + ",appid int" + ",dnisid int" + ",isconsult bit" + ",isiscc bit" + ",hasconstr bit"
                    + ",hasdestr bit" + ",calltype int" + ")\n;");

            Wheres wh = new Wheres();
            wh.addWhere(
                    IQuery.getCheckedWhere("ThisDNid", ReferenceType.DN, FindNode(repComponents.getRoot(),
                            DialogItem.TLIB_CALLS, DialogItem.TLIB_CALLS_TEVENT, DialogItem.TLIB_CALLS_TEVENT_DN)),
                    "OR");
            wh.addWhere(
                    IQuery.getCheckedWhere("otherDNID", ReferenceType.DN, FindNode(repComponents.getRoot(),
                            DialogItem.TLIB_CALLS, DialogItem.TLIB_CALLS_TEVENT, DialogItem.TLIB_CALLS_TEVENT_DN)),
                    "OR");
            ...
            if (DatabaseConnector.TableExist("connid_logbr")) {
                ...
                getAllResults.add(new Pair<>("Unique calls in SIPServer and/or TServer",
            ...
            DatabaseConnector
                    .runQuery("create index idx_" + tmpTable1 + "connID on " + tmpTable1 + "(connectionidid);");
            ...
            // DatabaseConnector.runQuery("alter table " + tmpTable1 + "tlibid on " +
            // tmpTable1 + "(tlibid);");
            ...
            DatabaseConnector.runQuery("WITH a AS\n" + "(\n" + "    SELECT\n" + "        " + tab + ".connectionIDid,\n"
                    ...
                    // + "\nwhere " + tmpTable + ".connectionidid=a.connectionidid)"
                    // + ","
                    // + "\nisconsult = \n(select transferidid>0 and transferidid<>connectionidid
                    // from a "
                    // + "\nwhere " + tmpTable + ".connectionidid=a.connectionidid)"
                    // + ","
                    ...
                    // + "\nwhere " + tmpTable + ".connectionidid=a.connectionidid)"
                    // + "\nwhere connectionidid in (select a.connectionidid from a where
                    // a.connectionidid=" + tmpTable + ".connectionidid)"
                    // + ";");
    ...
    public void addTLibReportType(DynamicTreeNode<OptionNode> root) {
        ...
        // AttrValue = new DynamicTreeNode<OptionNode>(new OptionNode(false,
        // "EventQueued"));
        ...
        root.addChild(nd);
        nd.addDynamicRef(DialogItem.TLIB_CALLS_TIMER_NAME, ReferenceType.TEvent, TableType.TLIBTimerRedirect.toString(),
                "msgid");
        nd.addDynamicRef(DialogItem.TLIB_CALLS_TIMER_CAUSE, ReferenceType.TEventRedirectCause,
                TableType.TLIBTimerRedirect.toString(), "causeID");
    ...
    // private void addSIPReportType(DynamicTreeNode<OptionNode> root) {
    // DynamicTreeNode<OptionNode> nd = new DynamicTreeNode<OptionNode>(new
    // OptionNode(true, "SIP"));
    // root.addChild(nd);
    //
    // DynamicTreeNode<OptionNode> ComponentNode;
    ...
    private void loadStdOptions(QueryDialogSettings qdSettings) throws SQLException {
        ...
        if (node != null) {
            ...
            if (node1 != null) {
                ...
                node1.addDynamicRef(DialogItem.SIP1536_SIP_MESSAGE, ReferenceType.SIPMETHOD,
                        TableType.SIP1536ReqResp.toString(), "msgid");
            }
            node1 = getNode(DialogItem.SIP1536_TRUNK, TableType.SIP1536Trunk.toString());
            if (node1 != null) {
                node.addChild(node1);
                node1.addDynamicRef(DialogItem.SIP1536_TRUNK_NAME, ReferenceType.DN, TableType.SIP1536Trunk.toString(),
                        "trunkid");
            ...
            if (node1 != null) {
                node.addChild(node1);
                node1.addDynamicRef(DialogItem.SIP1536_MISC_KEY, ReferenceType.Misc, TableType.SIP1536Other.toString(),
                        "keyid");
            }
    ...
    public ArrayList<NameID> getApps() throws SQLException {
        if (appsType == null) {
            // appsType = getAppsType(FileInfoType.type_TransportLayer, new
            // String[]{"tlib_logbr", "fileid", "sip_logbr", "fileid"});
    ...
    private void RetrieveSIP(QueryDialog dlg, DynamicTreeNode<OptionNode> reportSettings, IDsFinder cidFinder)
            throws SQLException {
    ...
    private void RetrieveTLib(QueryDialog dlg, DynamicTreeNode<OptionNode> eventsSettings, IDsFinder cidFinder)
        ...
        if (isChecked(eventsSettings) && DatabaseConnector.TableExist("tlib_logbr")) {
            ...
            if (cidFinder != null && callRelatedSearch(cidFinder) && !refIDs.isEmpty()) {
                ...
                getRecords(tlibQuery);
                if (!idsReqs.isEmpty()) {
                    Integer[] textID = DatabaseConnector.getIDs("trigger_logbr", "textid",
                            getWhere("tlibid", idsReqs, true));
                    if (textID != null && textID.length > 0) {
                        handlerIDs.addAll(
                                DatabaseConnector.getIDs("handler_logbr", "id", getWhere("textid", textID, true)));
                ...
                // DatabaseConnector.TableExist(ReferenceType.TEvent.toString())) {
                ...
                // + getWhere(TLibReq.getTabAlias() + ".nameid", ReferenceType.TEvent,
                // RequestsToShow, false)
                // + "\n and \n"
                ...
                // TLibReq.AddCheckedWhere("attr1id", ReferenceType.TLIBATTR1, eventsSettings,
                // "AND", DialogItem.TLIB_CALLS_TEVENT_ATTR1);
                // TLibReq.AddCheckedWhere("attr2id", ReferenceType.TLIBATTR2, eventsSettings,
                // "AND", DialogItem.TLIB_CALLS_TEVENT_ATTR2);
                //
    ...
    private void RetrieveTLibTimer(QueryDialog dlg, DynamicTreeNode<OptionNode> eventsSettings, IDsFinder cidFinder)
            throws SQLException {
        Integer[] connIDs;
        if (cidFinder == null) {
            ...
            if (isChecked(eventsSettings) && DatabaseConnector.TableExist(TableType.TLIBTimerRedirect.toString())) {
                ...
                if (DatabaseConnector.TableExist(TableType.TLIBTimerRedirect.toString())
                        && (cidFinder.getSearchType() == SelectionType.NO_SELECTION || !isEmpty(connIDs))) {
                    ...
                    TLibTimer.addRef("causeID", "cause", ReferenceType.TEventRedirectCause.toString(),
    ...
    private void RetrieveCommonLib(QueryDialog dlg, DynamicTreeNode<OptionNode> eventsSettings, IDsFinder cidFinder)
            throws SQLException {
    ...
    private void RetrieveSIPJson(QueryDialog dlg, DynamicTreeNode<OptionNode> eventsSettings, IDsFinder cidFinder)
            throws SQLException {
    ...
    private void RetrieveSIPDiagnostics(QueryDialog dlg, DynamicTreeNode<OptionNode> eventsSettings,
        ...
        if (isChecked(eventsSettings) && DatabaseConnector.TableExist(TableType.SIP1536Other.toString())) {
            ...
            if (isChecked(node)) {
                ...
                for (int i = 1; i <= com.myutils.logbrowser.indexer.SingleThreadParser.MAX_1536_ATTRIBUTES; i++) {
                    ...
                    sip1536Table.addRef(fldName + "id", fldName, ReferenceType.Misc.toString(),
                            IQuery.FieldType.OPTIONAL);
                    // fldName = "value" + i;
                    // ORSAlarm.addRef(fldName + "id", fldName, ReferenceType.Misc.toString(),
                    // IQuery.FieldType.Optional);
                }
                sip1536Table.addRef("keyid", "key", ReferenceType.Misc.toString(), IQuery.FieldType.OPTIONAL);
                sip1536Table.AddCheckedWhere(sip1536Table.getTabAlias() + ".keyid", ReferenceType.Misc, node, "AND",
            ...
            if (isChecked(node)) {
                ...
                sip1536Table.AddCheckedWhere(sip1536Table.getTabAlias() + ".trunkid", ReferenceType.DN, node, "AND",
    ...
    private void RetrieveISCC(QueryDialog dlg, DynamicTreeNode<OptionNode> eventsSettings, IDsFinder cidFinder)
            throws SQLException {
    ...
    private void runSelectionQuery(QueryDialog dlg, SelectionType selectionType, IDsFinder cidFinder)
            throws SQLException {
        handlerIDs.clear();
                ...
                if (selectionType != SelectionType.NO_SELECTION && !cidFinder.anythingSIPRelated()
                    ...
                    if (!isSuppressConfirms()) {
                        int dialogResult = inquirer.showConfirmDialog(dlg,
                                "No SIP calls found.\n" + "Do you want to show all SIP messages?", "Warning",
        ...
        RetrieveSIPDiagnostics(dlg, FindNode(repComponents.getRoot(), DialogItem.SIP1536, null, null), cidFinder);
